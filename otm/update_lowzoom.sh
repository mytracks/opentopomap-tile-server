...
if [ -f /external-data/lowzoom-update-complete ]; then
    echo "Skipping Lowzoom data update: data already updated"
    exit 0
fi

echo "Update Lowzoom data"

psql gis < /usr/src/OpenTopoMap/mapnik/tools/arealabel.sql
...
touch /external-data/lowzoom-update-complete
